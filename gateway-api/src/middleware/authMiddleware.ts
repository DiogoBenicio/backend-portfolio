...
export async function authMiddleware(
  ...
  if (url === "/health" || url === "/api/v1/health") return;
  ...
  try {
    ...
    if (err instanceof TokenExpiredError) {
      ...
      return reply.status(401).send({
        ...
        error: "Token Expired",
        message:
          "Token expirado. Faça login novamente ou use /api/auth/refresh",
      });
    }
    if (err instanceof JsonWebTokenError) {
      logger.warn(
        `Auth: token inválido  [${method} ${url}]  ip=${request.ip}  reason=${(err as Error).message}`,
      );
      return reply.status(401).send({
        status: 401,
        error: "Invalid Token",
    ...
    logger.error(
      `Auth: erro inesperado  [${method} ${url}]  ip=${request.ip}`,
      err,
    );
    throw err;
  }
